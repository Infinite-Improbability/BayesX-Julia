...
struct PlaintextData{S<:AbstractString,T<:DimensionfulAngles.Angle} <: Dataset
    observation::S
    background::S
    arf::S
    rmf::S
...
"""
    load_data(data::PlaintextData, shape, obs_exposure_time, bg_exposure_time)
...
"""
function load_data(data::PlaintextData, shape::NTuple{2,Int}, obs_exposure_time::Unitful.Time, bg_exposure_time::Unitful.Time)::NTuple{2,Pair}
    obs_data = reshape(readdlm(data.observation, Int), shape)
    bg_data = reshape(readdlm(data.background, Int), shape)
    return (obs_data => obs_exposure_time, bg_data => bg_exposure_time)
end

"""
    load_response(data::PlaintextData, rmf_shape, unit)
...
"""
function load_response(data::PlaintextData, rmf_shape::NTuple{2,Int}, unit::Unitful.Area=1u"cm^2")::NTuple{2,Pair}
    rmf = reshape(readdlm(data.rmf), rmf_shape)
    arf = reshape(readdlm(data.arf), rmf_shape[2])
