...
"""
    bin_energy(response_function, energy_bins, bin_size)

Reduce the number of energy bins by binning the response matrix.
Return `(new_response_function, new_energy_bins)`.

Response matrix is expected to be in the format R(PI,E).
"""
function bin_energy(response_function::Matrix{T}, energy_bins::Vector{E}, bin_size::Int)::Tuple{Matrix{T},Vector{E}} where {T<:Unitful.Area{Float64},E<:Unitful.Energy}
    @argcheck bin_size > 0

    if bin_size == 1
        @mpidebug "Skipping energy binning" bin_size
        return response_function, energy_bins
    end

    @mpidebug "Binning energy" bin_size

    slice_indices = StepRange(1, bin_size, size(response_function, 2))

    new_resp = Matrix{T}(undef, size(response_function, 1), length(slice_indices))
    new_bins = Vector{E}(undef, length(slice_indices) + 1)

    for i in eachindex(slice_indices)
        if i == length(slice_indices)
            slice = response_function[:, slice_indices[i]:end]
            new_bins[end] = energy_bins[end]
        else
